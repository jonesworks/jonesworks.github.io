...
```{r foo20, fig.align = 'center', message=F, warning=F, echo=FALSE}
library(ggplot2)
p5 = ggplot(train, 
...
Below are the results from a few very preliminary analyses. Linear models were used to examine relationships between new features and the rate at which individuals were monitored and kicked. Again, below is the result of one analysis from many. 

Important to note is that the resultant analysis is simple in the extreme: it employs  linear regression. 

As is well known, simple models are generally the most interpretable, but the in this case, the analysis would strike some analysts as so simple as to seem trivial.

Nonetheless, it facilitates the development of a fingertip feel for the data set in terms of relationships between variables. 

Here, we see the relation between event category, duration of the given event, and the significance tests sheds light on whether an event, due to its category and duration, results in a relative increase in the number of attendees ejected.

Though not published in this report, elsehwere, conducted similar regression analysis, we find that bouncer is a significant predictor of the rate at which attendees are kicked. In other words, the bouncer working an event explains much of the variance in the data, with respect to attendees kicked and monitored.

This observation, the only useful observation at this point, points us a way forward, for lack of a better turn of phrase. It would make sense to conduct a careful analysis of bouncer as it relates to rate of being kicked and, due to the relatively sparsity of the data set, request additional information about the bouncers. 

```{r foo7, fig.align = 'center', message=F, warning=F, echo=FALSE}
library(stargazer)
contrasts(duration_and_kicked_and_categories$event.categories) = "contr.sum"
m.lm <- lm(kicked ~ duration + event.categories, 
           data=duration_and_kicked_and_categories)
summary(m.lm)
